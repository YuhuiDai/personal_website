(function (root, factory) {
  ...
  d3.svg.BubbleChart = function (settings) {
    ...
    $.extend(self.options, {
      ...
      intersectDelta: 0,
      radiusMax: (defaultOuterRadius-defaultInnerRadius)/2,
      intersectInc: 0,
  ...
  $.extend(d3.svg.BubbleChart.prototype, {
    ...
    setup: function () {
      ...
      var fnColor = d3.scale.quantize()
                      .domain([1, 4])
                      .range(["#99ccff", "#ffd202","#ff8f97", "#d3ccff"]);
      
      node.append("circle")
        .attr({r: function (d) {return d.r;}, cx: function (d) {return d.cx;}, cy: function (d) {return d.cy;}})
        .style("fill", function (d) {
          
          return options.data.color !== undefined ? options.data.color(d.item) : fnColor(d.item.category);
        })
        .attr("opacity", "0.8");
    ...
    moveToCentral: function (node) {
      ...
      var toCentralPoint = d3.svg.transform()
        .translate(function (d) {
          ...
          console.log("calling move to central");
          return [dx, dy];
        });
    ...
    moveToReflection: function (node, swapped) {
      ...
      var toReflectionPoint = d3.svg.transform()
        .translate(function (d) {
          ...
          console.log("calling move to reflection");
          return [dx, dy];
        });
